...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, priors, targets):
        ...
        '''
            loc_data的shape为(B, SSD512priorbox的数量:32756, 4)
            conf_data的shape为(B, 32756, 2)
            priors的shape为(32756, 4)
            targets为长度是B的list，每个list的元素的shape为：(该图片中gt数, 5)
        '''
        ...
        N = max(num_pos.data.sum().float(), 1)  # 可能会出现0，所以取一个大于1的值
