...
def _crop_old(image, boxes, labels):
...
def _expand_old(image, boxes, fill, p):
    if random.random() > p:
        return image, boxes
...
def preproc_for_test_old(image, insize, mean, std=(1, 1, 1)):
...
class preproc_old(object):
    ...
    def __call__(self, image, targets):
        ...
        if len(boxes) == 0:
            ...
            image = preproc_for_test_old(image, self.resize, self.means, self.std)
        ...
        # image_t, boxes, labels = _crop_old(image, boxes, labels)
        # image_t = _distort_old(image_t)
        image_t = _distort_old(image)
        image_t, boxes = _expand_old(image_t, boxes, self.means, self.p)
        image_t, boxes = _mirror_old(image_t, boxes)
        # image_t, boxes = _mirror(image, boxes)

        height, width, _ = image_t.shape
        image_t = preproc_for_test_old(image_t, self.resize, self.means, self.std)
        boxes = boxes.copy()
        boxes[:, 0::2] /= width
        boxes[:, 1::2] /= height
        ...
        if len(boxes_t) == 0:
            image = preproc_for_test_old(image_o, self.resize, self.means, self.std)
...
class BaseTransform_old(object):
