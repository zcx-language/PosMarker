...
class Shufflenet(nn.Module):
    ...
    def forward(self, old_x):
        if self.stride == 1:
            ...
            tmp = torch.cat((x_proj, self.branch_main(x)), 1)
            # print("shufflenet", "stride:", self.stride, "shape:", tmp.shape)
            return tmp
        elif self.stride == 2:
            x_proj = old_x
            x = old_x
            tmp = torch.cat((self.branch_proj(x_proj), self.branch_main(x)), 1)
            # print("shufflenet", "stride:", self.stride, "shape:", tmp.shape,
            #       "branch_proj:", self.branch_proj(x_proj).shape,
            #       "branch_main:", self.branch_main(x).shape)
            return tmp
...
class Shuffle_Xception(nn.Module):
    ...
    def forward(self, old_x):
        if self.stride == 1:
            x_proj, x = channel_shuffle(old_x)
            tmp = torch.cat((x_proj, self.branch_main(x)), 1)
            # print("shufflenet_x", "stride:", self.stride, "shape:", tmp.shape)
            return tmp
        elif self.stride == 2:
            x_proj = old_x
            x = old_x
...
def channel_shuffle(x):
    ...
    return x[0], x[1] # 将channels等分成两部分
